...
qactorSystemSpec:
	 systemSpec  WS 
	( mqttBroker = brokerSpec)? WS 
	( message   += messageP+   ) WS 
	( context   += contextP+   ) WS 
	( actor     += qActorDeclaration+    ) WS EOF
; 
*/


qactorSystemSpec: systemSpec brokerSpec? messageSpec* contexSpec* actorSpec* EOF; 

   
systemSpec  : SYSTEMSTRING  ID (name=ID) ;
brokerSpec  : MqttBrokerSTRING  STRING (brokerHost=STRING) DuePuntiSTRING INT (brokerPort=INT) ; 
messageSpec : messageP (message+=messageP+) ;
contexSpec  : contextP (context+=contextP+) ;
actorSpec   : qActorDeclaration (actor+=qActorDeclaration+) ;
...
event: 		EventSTRING				ID name=ID  DuePuntiSTRING  pHead msg=pHead    ;
signal: 	SignalSTRING			ID name=ID  DuePuntiSTRING  pHead msg=pHead   ;
token:		TokenSTRING				ID name=ID  DuePuntiSTRING  pHead msg=pHead   ;
dispatch: 	DispatchSTRING			ID name=ID  DuePuntiSTRING  pHead msg=pHead  ;
request: 	REQUESTCAPITALSTRING	ID name=ID  DuePuntiSTRING  pHead msg=pHead  ;
reply: 	    ReplySTRING				ID name=ID  DuePuntiSTRING  pHead msg=pHead  ;
invitation:	InvitationSTRING		ID name=ID  DuePuntiSTRING  pHead msg=pHead    ;
...
;  
ifSolvedAction     : IfSolvedSTRING OpenBraceSTRING ( solvedactions += stateAction )*  CloseBraceSTRING //action=ActionSequence 
					 (ElseSTRING  OpenBraceSTRING ( notsolvedactions += stateAction )*  CloseBraceSTRING)?
;
guardedStateAction : IfSTRING guard = anyAction OpenBraceSTRING ( okactions += stateAction )*  CloseBraceSTRING  //action=ActionSequence  
...
solveGoal      :  SolveSTRING OpenRoundSTRING goal=pHead (VirgilaSTRING resVar=variableX)? CloseRoundSTRING;
...
msgCond   :	OnMsgSTRING OpenRoundSTRING message=messageP DuePuntiSTRING msg = pHead CloseRoundSTRING OpenBraceSTRING ( condactions += stateAction )*  CloseBraceSTRING
...
anyAction : OpenSquareSTRING body=KCODE CloseSquareSTRING  ; 							//OpenSquareSTRING body=STRING CloseSquareSTRING; 
